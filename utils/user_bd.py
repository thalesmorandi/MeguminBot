...
async def get_local_rep(user_id, server_id):
    try:
        conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
        localrep = await conn.fetch('''SELECT localrep FROM serverusers WHERE serverid ={1} and memberid ={0}'''.format(user_id, server_id))       
        localrepr = localrep[0]
        localrep = localrepr['localrep']
        await conn.close()
        return localrep
    except:
        await conn.close()     
...
async def set_rep(server_id, user_id: int, repadd: int):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    rep = await conn.fetch('''SELECT rep FROM public.users WHERE ID ={}'''.format(user_id))       
    repr = rep[0]
    rep = repr['rep']
...
async def unset_xp(user_id, xpadd: int):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    xp = await conn.fetch('''SELECT xp FROM public.users WHERE ID ={}'''.format(user_id))       
    xpr = xp[0]
    xp = xpr['xp']
...
async def unset_eris(user_id: int, erisadd: int):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    eris = await conn.fetch('''SELECT eris FROM public.users WHERE ID ={}'''.format(user_id))       
    erisr = eris[0]
    eris = erisr['eris']
...
async def get_bio(user_id: int):
    try:
        conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
        bio = await conn.fetch('''SELECT bio FROM public.users WHERE ID ={}'''.format(user_id))       
        bior = bio[0]
        bio = bior['bio']
        await conn.close()        
        return bio
...
async def set_bio(user_id, bio):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    await conn.fetch('''UPDATE public.users SET bio = '{1}' WHERE ID = {0}'''.format(user_id, bio))       
    await conn.close()
...
async def unset_local_xp(server_id, user_id, xpadd: int):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    xp = await conn.fetch('''SELECT localxp FROM public.serverusers WHERE serverid ={} and memberid ={}'''.format(server_id, user_id))       
    xpr = xp[0]
    xp = xpr['localxp']
...
async def get_ranking_pos(member_id):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    ranking = await conn.fetch('''SELECT id, xp FROM public.users ORDER BY xp DESC''')       
    await conn.close()
    for _ in range(0, len(ranking)):
...
async def get_local_ranking_pos(server_id, member_id):
    conn = await asyncpg.connect(user=dbuser, password=dbpassword, database=dbdatabase, host=dbhost)        
    ranking = await conn.fetch('''SELECT memberid, localxp FROM public.serverusers WHERE serverid = {} ORDER BY localxp DESC'''.format(server_id))       
    await conn.close()
    for _ in range(0, len(ranking)):
        rankingmember = ranking[_]
        id = rankingmember['memberid']
        if id == int(member_id):
            pos = _+1
            return pos
